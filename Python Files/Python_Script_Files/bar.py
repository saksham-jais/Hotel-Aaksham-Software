...
customtkinter.CTkLabel(master=frame, text="Search By City :", font=('Century Gothic', 16)).place(x=20, y=30)
...
Entry(frame,highlightthickness=2,highlightbackground="grey",highlightcolor="black",fg="blue",font="consolas 17 italic").place(x=280,y=30,width=200,height=30)
Entry(frame,highlightthickness=2,highlightbackground="grey",highlightcolor="black",fg="blue",font="consolas 17 italic").place(x=280,y=80,width=200,height=30)
Entry(frame,highlightthickness=2,highlightbackground="grey",highlightcolor="black",fg="blue",font="consolas 17 italic").place(x=280,y=130,width=200,height=30)

tkinter.Button(frame, image=Guest_Entry, compound=LEFT, fg="Black", width=120, activeforeground="black",activebackground="#a8701d", height=30, text="Search", bg="#a8701d", anchor=W,font=('Century Gothic', 17, "bold"), borderwidth=5, cursor="hand2").place(x=500+20, y=30)
tkinter.Button(frame, image=Guest_Entry, compound=LEFT, fg="Black", width=120, activeforeground="black",activebackground="#a8701d", height=30, text="Reset", bg="#a8701d", anchor=W,font=('Century Gothic', 17, "bold"), borderwidth=5, cursor="hand2").place(x=650+20, y=30)
...
table = ttk.Treeview(frm1, cursor="hand2", columns=("SN_No.", "Gs_ID", "Gs_Name", "Gender","Religion","Address","City","Country","Contact_No","ID_Type","ID_No","Email_ID"),
...
query = "select* from `Customer Details` order by `S No.`;"
cur.execute(query)
for row in cur.fetchall():
    table.insert("",END, values=(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10],row[12]))
def table_select(_):
    fu=[]
