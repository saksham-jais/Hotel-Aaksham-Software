...
Label(text="List Of Check In",background="brown",font=('Times New Roman',30,"bold")).pack(anchor=N,fill=X)
...
def Scrh():
    if E1.get()==""and E2.get()=="":
        for item in table.get_children():
            table.delete(item)
        query = f"select * from `check in details`  right join `customer details` using (`Guest ID`) where `check in details`.`Room No` is not null and `Contact No.`='{E3.get()}';"
        cur.execute(query)
        sn = 1
        for row in cur.fetchall():
            table.insert("", END, values=(
            sn, row[0], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[12], row[9], row[10], row[15],
            row[16], row[17], row[18], row[19], row[20], row[24]))
            sn += 1
    elif E2.get()==""and E3.get()=="":
        for item in table.get_children():
            table.delete(item)
        query = f"select * from `check in details`  right join `customer details` using (`Guest ID`) where `check in details`.`Room No` is not null  and `Guest ID`='{E1.get()}';"
        cur.execute(query)
        sn = 1
        for row in cur.fetchall():
            table.insert("", END, values=(
        ...
        for item in table.get_children():
            table.delete(item)
        query = f"select * from `check in details`  right join `customer details` using (`Guest ID`) where `check in details`.`Room No` is not null  and `Guest Name`='{E2.get()}';"
        cur.execute(query)
        sn = 1
        for row in cur.fetchall():
            table.insert("", END, values=(
...
def Rset():
    ...
    for item in table.get_children():
        table.delete(item)
    query = "select * from `check in details`  right join `customer details` using (`Guest ID`) where `check in details`.`Room No` is not null;"
    cur.execute(query)
    sn = 1
    for row in cur.fetchall():
        table.insert("", END, values=(
        sn, row[0], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[12], row[9], row[10], row[15], row[16],
        row[17], row[18], row[19], row[20], row[24]))
        sn += 1
...
def cls():
    ...
    so = pd.Series(data=fu, name="ChkIn")
    sep = pd.DataFrame(so)
    sep.to_csv("./CSV_FILE/Checkin_list.csv")
    root.destroy()
...
table = ttk.Treeview(frm1, cursor="hand2", columns=("SN_No.", "Gs_ID", "Gs_Name", "Gender","Religion","Address","City","Country","Contact_No","Email Id","ID_Type","ID_No","Room_No","Room_Type","Date_In","Date_Out","Room_Charges","Payment","Booking_Type",'Image'),
...
table.heading("Email Id", text="Email Id", anchor=CENTER)
...
table.heading("Booking_Type", text="Booking Type", anchor=CENTER)
...
table.column("Email Id", width=160, anchor=CENTER, minwidth=140)
...
table.column("Booking_Type", width=140, anchor=CENTER, minwidth=120)
table.column("Image", width=0, anchor=CENTER, minwidth=0)
query = "select * from `check in details`  right join `customer details` using (`Guest ID`) where `check in details`.`Room No` is not null;"
cur.execute(query)
sn=1
for row in cur.fetchall():
    table.insert("",END, values=(sn,row[0], row[2], row[3], row[4], row[5], row[6], row[7], row[8],row[12], row[9], row[10],row[15], row[16], row[17], row[18], row[19],row[20],row[24],row[13]))
    sn+=1
# query = "select * from `check in details`  right join `customer details` using (`Guest ID`);"
# cur.execute(query)
# for row in cur.fetchall():
#     table.insert("",END, values=(row[1], row[0], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[13], row[14], row[15], row[16], row[17], row[18]))

def table_select(_):
    fu=[]
    for i in table.selection():
        fu.append(table.item(i)['values'])
    so = pd.Series(data=fu[0], name="ChkIn")
    sep = pd.DataFrame(so)
    sep.to_csv("./CSV_FILE/Checkin_list.csv")
    # print(so)
    root.destroy()
